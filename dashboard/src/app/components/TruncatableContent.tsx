...
const TruncatableContent: React.FC<TruncatableContentProps> = ({ title, content, initialLineLimit = 3, initialCharLimit = 300 }) => {
  ...
  return (
    <div className="mb-3 p-3 border border-slate-200 rounded-md bg-white">
      <h4 className="text-md font-semibold text-slate-700 mb-1">{title}</h4>
      <pre className="text-sm text-slate-600 bg-slate-50 p-2 rounded whitespace-pre-wrap font-sans">
        {displayedContent}
      ...
      {isActuallyTruncated && (
        <button
          ...
          className="text-xs text-blue-600 hover:text-blue-800 hover:underline mt-1 focus:outline-none"
        >
